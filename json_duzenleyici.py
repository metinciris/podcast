...
class CustomWordsEditor(tk.Tk):
    ...
    def save_json_file(self):
        # Kaydederken daima harf sırasıyla json içine yaz
        sorted_dict = {k: self.custom_words[k] for k in sorted(self.custom_words, key=lambda x: x.lower())}
        with open(self.json_filename, "w", encoding="utf-8") as f:
    ...
    def update_word_list(self):
        ...
        # Listbox'ta da harf sırası korunsun
        for word in sorted(self.custom_words.keys(), key=lambda x: x.lower()):
            self.listbox.insert(tk.END, f"{word}  →  {self.custom_words[word]}")
